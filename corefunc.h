...
public:
	...
	//Discord
	/*
	bool dcmsg(string message) {
		try {
		string token = "OTY3MjkxOTc3NTU5MzMwODQ2.YmOK2g.zUVEk0JHqdxX3FoBp13y6ru2rq4";
		string channelid ="967614056900988998";
		string endpoint = "https://discord.com/api/v6/channels/" + channelid + "/messages";
		cpr::Response x = cpr::Post(cpr::Url{ endpoint },
		cpr::Header{ {"Content-Type", "application/json"}, {"Authorization", token} },
		cpr::Body{ "{\"content\":\"" + message + "\"}" });
		return true;
		}
		catch (exception ex)
		{
	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("type|local") != std::string::npos) {
	    				...
	    				cout << uname << " Local netid: " << localid << " X: " << localX / 32 << " Y: " << localY / 32 << endl;
					}	
					else if (ctx.find("spawn|avatar") != std::string::npos)  {
						string namaplayer = explode("\n", explode("name|`w", ctx)[1])[0];
						cout << "Player Joined : " << namaplayer  << endl;
						string joined = "Player Joined : "+namaplayer;
						wLog(joined);
					}
					...
					auto ctx = varlist[1].get_string();
					/*
					if (ctx.find("CP:0_PL:0_OID:_CT:[W]_") != std::string::npos) {
						string player = explode("\n", explode("CP:0_PL:0_OID:_CT:[W]_ `6<`w", ctx)[1])[0];

					}*/
					//Vending 
					if (ctx.find("`7[```9") != std::string::npos) {
						string vndlog = explode("\n", explode("`7[```9", ctx)[1])[0];
						//string vndlog = explode(vndlog,"```7]``");
						cout << "[[ "+vndlog << endl;
						wLog("[[ "+vndlog);
						
					}
					//donation box
					//`7[```5[```wFJF6 places `510`` `2Wheat`` into the Donation Box`5]```7]``
					else if (ctx.find("`7[```5[```w") != std::string::npos) {
						string donatelog = explode("\n", explode("`7[```5[```w", ctx)[1])[0];
						//string donatelog = exlplode(donatelog,"```7]``");
						cout << "[[ "+donatelog << endl;
						wLog("[[ "+donatelog);
